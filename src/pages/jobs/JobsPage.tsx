...
const JobsPage: React.FC = () => {
  ...
  const loadJobs = async (pageNum: number, reset = false) => {
    ...
    setLoading(true);
    try {
      const response = await fetchJobs(pageNum, filters, {
        field: sortOption,
      ...
      <div className="bg-white shadow-sm sticky top-0 z-40">
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-xl font-bold text-gray-900 flex-1">全部职位</h1>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              value={sortOption}
              onChange={(e) => setSortOption(e.target.value as SortOptions['field'])}
              className="text-sm border-none bg-transparent text-primary-600 focus:outline-none"
            >
        ...
        {loading && page === 1 ? (
          ...
          <div className="bg-white rounded-xl p-8 text-center">
            ...
            <h3 className="text-gray-700 font-medium mb-2">暂无职位</h3>
            <p className="text-gray-500 text-sm">没有找到符合条件的职位，请尝试调整筛选条件</p>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            {hasMore && (
              <button
                ...
                disabled={loading}
                className="w-full py-4 text-center text-primary-600 hover:text-primary-700 font-medium disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading ? (
