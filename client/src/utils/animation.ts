import type { FissionConfig, Particle, ShapeType } from '~/types/geometric';

export function createParticles(config: FissionConfig, centerX: number, centerY: number): Particle[] {
...
export function updateParticles(
  ...
  return particles.map((particle) => {
    ...
    const distFromCenter = Math.sqrt(
      Math.pow(newParticle.x - centerX, 2) + Math.pow(newParticle.y - centerY, 2)
    );
