...
export const FissionCanvas = component$(({ config, isPlaying, onReset$ }) => {
    const canvasRef = useStore({ canvas: null });
    ...
    const draw = $(() => {
        ...
        state.particles.forEach((particle) => {
            ...
            drawShape(ctx, particle.shapeType, particle.x, particle.y, particle.size, particle.rotation, particle.color, particle.opacity);
        });
    });
    const animate = $(() => {
        if (!canvasRef.canvas)
            return;
        state.particles = updateParticles(state.particles, config, canvasRef.canvas.width, canvasRef.canvas.height);
        draw();
        if (isPlaying) {
