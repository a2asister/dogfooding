<template>
  <div class="archive-view">
    <div class="page-header">
      <h2 class="page-title">归档箱</h2>
      <p class="page-subtitle">这里存放着曾经的灵感火花</p>
    </div>
    ...
    <div class="toolbar" v-if="selectMode">
      <div class="toolbar-info">
        ...
        <div class="toolbar-actions">
          <button class="toolbar-btn restore" @click="handleBatchRestore">恢复</button>
    ...
    <div class="notes-grid">
      ...
      <div v-if="notes.length === 0 && !loading" class="empty-state">
        ...
        <p class="empty-text">归档箱是空的</p>
        <p class="empty-hint">从灵感列表中选择笔记进行归档</p>
      </div>
...
async function loadNotes() {
  loading.value = true
  try {
    const params = { ...searchParams.value, archived: 'true' }
...
async function handleBatchRestore() {
  ...
  try {
    await batchArchiveNotes(selectedIds.value, false)
    exitSelectMode()
    loadNotes()
  } catch (error) {
